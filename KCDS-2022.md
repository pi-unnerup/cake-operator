...
Now go and have a look at your creation! If you are running on the workshop-provided cluster go to your browser and see your cake on `<node-IP>:8099`
...
If you are running on a local cluster such as Rancher, you will likely see it on `localhost:8099`.

Your cake should look something like this:
![cake rendering](./img/plain-cake.png)

Now lets change the port of our cake to 8088. In `config/samples/tutorials_v1_cake.yaml` change the port to 8088 and apply your cake again:

```sh
kubectl apply -f config/samples/tutorials_v1_cake.yaml
```

Your cake will now be available on port 8088.
...
We now have two options: we can write a reconciliation block for DECORATION and reinstall the operator or we can delete the cake resource and recreate it - in the interest of time, we will do the latter. 
...
You can submit your cake now (see section 4), or move onto the next section where we will update the operator to include an additional spec.

# 3. (Optional) Updating the Operator
...
Time to submit your cake! Take a screenshot of your cake and send it to xxxxx